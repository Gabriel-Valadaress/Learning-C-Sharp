...
namespace Linq02
{
    public class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var result1 = products.Where(p => p.Category.Tier == 1 && p.Price < 900).OrderBy(p => p.Price);
            ...
            var result3 = products.Where(p => p.Name.StartsWith('C')).Select(p => new { p.Name, p.Price, CategoryName = p.Category.Name });
            Print("NAMES STARTED WITH 'C' AND ANONYMOUS OBJECT:", result3);

            var result4 = products.Skip(2).Take(4).OrderBy(p => p.Name);
